  ...
  if (!isReadonly(target)) {
    {
      ...
      if (existingOb && existingOb.shallow !== shallow) {
        warn$2(
          `Target is already a ${existingOb.shallow ? '' : 'non-'}shallow reactive object, and cannot be converted to ${
            shallow ? '' : 'non-'
          }shallow.`
        )
      }
      ...
      return (
        ...
        tree
          .map(
            (vm, i) =>
              `${i === 0 ? '---> ' : repeat(' ', 5 + i * 2)}${
                isArray(vm) ? `${formatComponentName(vm[0])}... (${vm[1]} recursive calls)` : formatComponentName(vm)
              }`
          )
          .join('\n')
      )
